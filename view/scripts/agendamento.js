document.addEventListener("DOMContentLoaded", function() {
  ...
  const url = "http://localhost:8080/agendamento/user/1"
  ...
  async function agendar() {
    ...
    const selectTime = new String(selectedTime);
    ...
    try {
        ...
        const response = await fetch('http://localhost:8080/agendamento', {
            ...
            body: JSON.stringify({
              tipoServico: service,
              dataAgendamento: selectedDateTime.toISOString().split('T')[0],
              horaAgendamento: selectTime,
              status: "PENDENTE",
              user: {
                id: 1
              }
        ...
        console.log('Agendamento bem-sucedido:', responseData);
        closePopup();
    } catch (error) {
